  ...
  return (
    <div className="relative w-[64px]" data-expanded={isExpanded}>
      ...
      <div className="relative">
        ...
        <div 
          className={`relative w-16 h-8 cursor-pointer rounded-lg group will-change-transform z-50 transition-colors duration-300 ${
            isExpanded ? 'bg-gray-600 text-white' : 'bg-transparent text-white'
          }`}
        ...
        {childrenArray.slice(1).map((child, index) => (
          <div 
            key={index} 
            className="absolute top-0 left-0 w-16 h-8 bg-gray-600 rounded-lg will-change-transform shadow-lg border border-gray-500"
            style={{
          ...
          >
            {React.cloneElement(child as React.ReactElement, {
              onClick: () => handleItemClick((child as React.ReactElement).props.onClick)
            })}
          </div>
        ))}
      </div>
